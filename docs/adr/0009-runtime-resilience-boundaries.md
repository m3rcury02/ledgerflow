...
- Status: Accepted
- Date: 2026-07-14
- Amended by: ADR 0010 (management isolation and bounded/coalesced readiness)
...
5. Liveness reports only internal process state. Readiness includes application readiness, PostgreSQL, enabled Kafka-adapter connectivity, and drain state. PostgreSQL is startup-critical. Kafka is startup-checked when Kafka adapters are enabled; its failure does not mutate durable outbox data. ADR 0010 moves these endpoints to a management-only listener and bounds/coalesces readiness dependency work.
...
- At-least-once duplicates remain possible. PostgreSQL uniqueness, outbox ownership, inbox event-ID/hash checks, and ADR 0011's semantic notification identity remain the side-effect controls; this ADR makes no exactly-once end-to-end claim.
