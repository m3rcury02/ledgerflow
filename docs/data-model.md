...
PostgreSQL is the source of truth. Flyway creates all structures, constraints, indexes, trigger functions, and seed accounts. Integration tests apply every migration to an empty PostgreSQL Testcontainer. `orders` and `idempotency_records` are implemented by `V001`; every later table in this document remains proposed and must not be inferred to exist.

## General conventions

- Order IDs are PostgreSQL 18 UUIDv7 values generated by the `uuidv7()` column default. Identifier strategies for later tables are selected with their implementation milestones.
