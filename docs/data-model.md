...
PostgreSQL is the source of truth. Flyway creates all structures, constraints, indexes, and trigger functions. Integration tests apply every migration to an empty PostgreSQL Testcontainer. `orders` and `idempotency_records` are implemented by `V001`; `payments` and `payment_attempt_history` are implemented by `V002`. Later tables remain proposed and must not be inferred to exist.

## General conventions

- Order, payment, and payment-attempt-history IDs are PostgreSQL 18 UUIDv7 values generated by `uuidv7()` column defaults. Provider request IDs are application-generated UUIDs persisted before I/O.
...
- `AUTHORIZED`, `CAPTURING`, capture-retry, capture-unknown, capture-declined, and `CAPTURE_CONFIRMED` require an authorization ID;
- `CAPTURE_CONFIRMED` requires a capture request ID, provider capture ID, and null failure code;
...
2. verify payment is `CAPTURE_CONFIRMED` and order is `PAYMENT_PROCESSING`;
3. insert the ledger transaction using unique `(PAYMENT_CAPTURE, payment_id)`;
4. insert the debit and credit entries;
5. transition payment to `CAPTURED` while retaining the provider capture ID;
...
| Capture is provider-confirmed in Milestone 4 | Payment is `CAPTURE_CONFIRMED` | Milestone 5 will run idempotent ledger/outbox finalization and only then use `CAPTURED` |
