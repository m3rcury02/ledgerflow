...
This is a curated index, not a duplicate archive. Detailed rationale lives in accepted ADRs and
the topic-specific engineering documents linked below. This page gives a technical interviewer a
fast, topic-organized entry point. When a decision changes, its primary source is updated first and
this index is kept in sync.
...
- **VPC interface/gateway endpoints instead of a NAT Gateway**
  ([AWS design](aws-terraform-design.md)) — cost-comparable to
...
- **Kafka and an identity provider are explicitly out of scope for the Terraform design**
  ([AWS design](aws-terraform-design.md)), unlike the local cluster's Kafka and Keycloak — modeled
  as placeholder variables with explicit comments, not fabricated infrastructure.
- **Deterministic fake provider as the AI assistant's default, real provider opt-in only**
  ([assistant design](ai-operations-assistant.md)) — no test, script, container image, or default
  configuration can trigger real API
...
- **Retrieval happens in the service layer before any model is called, and citations are grounded
  against what was actually retrieved** ([assistant design](ai-operations-assistant.md)) — makes
  "never invent a runbook citation" a
...
- **Fix real Checkov/Trivy findings where proportionate, accept narrowly-scoped documented
  suppressions only for genuine trade-offs** ([AWS design](aws-terraform-design.md)) — CloudWatch
  retention, RDS Performance
...
- **A one-line, narrowly-scoped directory skip for `deploy/kind/dependencies/`, and the deliberate
  decision *not* to do the same for Terraform** — the Kubernetes
...
The portfolio extensions were rebuilt from scratch rather than cherry-picked from
`archive/main-before-d2f1721-2026-07-17`, a prior attempt that was merged then reverted. The
archive has specific, identifiable defects, including committed binaries and unsupported evidence,
so none of its implementation was reused.
...
topic documents and drift out of sync with them over time. Follow a link above for the full
argument, alternatives considered, and the evidence behind each choice.
